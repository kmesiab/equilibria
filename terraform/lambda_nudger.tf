resource "aws_lambda_function" "nudger_lambda" {
  function_name = "nudgerFunction"
  runtime       = "go1.x"
  handler       = "main"
  timeout       = 30
  filename      = "../build/nudge_sms.zip"
...
# Security Group
#
resource "aws_security_group" "nudger_lambda_sg" {
  name        = "nudger_lambda_sg"
