resource "aws_lambda_function" "sms_status_lambda" {
  function_name = "smsStatusFunction"
  runtime       = "go1.x"
  handler       = "main"
  timeout       = 30
  filename      = "../build/status_sms.zip"
...
# Security Group
#
resource "aws_security_group" "status_lambda_sg" {
  name        = "status_lambda_sg"
