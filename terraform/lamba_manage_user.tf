resource "aws_lambda_function" "manage_user_lambda" {
  function_name = "manageUserFunction"
  runtime       = "go1.x"
  handler       = "main"
  timeout       = 30
  filename      = "../build/manage_user.zip"
...
resource "aws_security_group" "manage_user_lambda_sg" {
  name        = "manage_user_lambda_sg"
